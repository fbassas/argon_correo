...
class Gmail(object):
    ...
    def database(self):
        if self.__database is None:
            ...
            dbname = os.path.join('/var/tmp',self.username)
            print dbname
    ...
    def imap(self):
        ...
        if self.__imap is None:
            ...
            oauth2String = oauth2.GenerateOAuth2String(self.username, self.password, base64_encode=False)
            #self.__imap.login(self.username, self.password)
            #self.__imap.authenticate('XOAUTH2',lambda x: oauth2String)
            self.__imap.authenticate('XOAUTH2',lambda x:oauth2.GenerateOAuth2String(self.username, self.password, base64_encode=False)) 
...
def log(message):
    # Frank: treiem els ^M, que son retorns de carro, per evitar salts de linia
    clean_message = message.replace('\r','').replace('\n','')
    print '[%s]: %s' % (time.strftime('%H:%M:%S'), encode_unicode(clean_message))
    f_log.write('[{0}]: {1}\n'.format(time.strftime('%H:%M:%S'), encode_unicode(clean_message)))
...
def main():
    ...
    data_i_hora = (datetime.datetime.today()).strftime('%d%m%Y-%H%M')
    F_LOG = "/var/log/maildir2gmail/{0}_{1}.{2}.log".format(options.username,data_i_hora,os.path.basename(args[0]))
